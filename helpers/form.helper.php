...
class form {
  ...
  public function form_header($action_target, $method_target = 'post', $id = "", $validation = false, $js_actions = false, $enctype = null) { //Creates the Form Tag with action and method
    ...
    $valid_action = (($validation === true) ? "onsubmit=\"return Validator(this); return false;\" " : null);

    if ($js_actions["onsubmit"] && $validation === true)
      $valid_action = "onsubmit=\"return validHandler(this); return false;\" ";
    ...
    return '<form '.$form_action. $name . $method . $id_r . $f_class . $valid_action . $actions . $enctype .'>'.PHP_EOL;
  }
  ...
  public function make_field($field) { //Calls the right function to create a field with the proper type, some own types are included
    if (is_array($field)) {
      if (isset($field["type"])) {
        ...
        switch ($field['type']) {
          case 'textarea':
            $field = $this->make_textarea($field);
            break;
          case 'select':
            $field = $this->make_select($field);
            break;
          case 'checkbox':
            $field = $this->make_input_checkbox($field);
            break;
          case 'radio':
            $field = $this->make_input_radio($field);
            break;
          case 'button':
            $field = $this->make_input_button($field);
            break;
          default:
            $field = $this->make_input($field);
        }
  ...
  public function create_output() {  //Lists the "ready to use" forms and calls the make_field function
    ...
    $this->form_method = (empty($this->form_method) || strtolower($this->
      form_method) == 'post' ? 'post' : 'get');
    $this->form_result .= $this->form_header($this->form_target, $this->form_method,
      $this->form_id, $validate, $js_actions, $this->form_enctype);
  ...
  private function process_fieldset($array) {
    ...
    if (isset($array["fieldset"])) {
      ...
      if (isset($fieldset["info"]))
        $output .= "<p class='info'>" . $fieldset["info"] . "</p>";
  ...
  private function make_input($params) {//Creates a form value reset field
    ...
    $field_result = '<input ' . $this->create_params($params) . '/>'.PHP_EOL;
    if (!empty($description) && $params['type'] != "hidden")
  ...
  private function make_input_radio($params) //Creates a hidden field
  {
    ...
    $boxed = $params["boxed"];
    unset($params['label'], $params['description'], $params["name"], $params["boxed"],
      $params["type"]);
    foreach ($params["value"] as $value => $name) {
      (isset($params["checked"]) and $params["checked"] == $value) ? $checked =
  ...
  // This function takes the information of a table and creates
  // the right fields by passing the col-types and field-types together
  public function auto_create_form($table, $cols = null) {
    if (empty($cols)) {
    ...
    for ($item = 0; $item < count($cols); $item++) {
      //cycles through the fields and calls the make field
      ...
      if ($cols[$item]['Default'] != "CURRENT_TIMESTAMP" || $cols[$item]['Extra'] !="auto_increment") {
        ...
        switch ($cols[$item]['Type']) {
          ...
          case ("float"):
            $form .= "<label for='" . $cols[$item]['Field'] . "'>$name</label>
																	<input type='text' name='" . $cols[$item]['Field'] . "' id='" .
              $cols[$item]['Field'] . "' /><br />";
            break;

          case ("text"):
            $form .= '<label for="' . $cols[$item]['Field'] . '">'.$name.'</label>'
                  . '<textarea name="' . $cols[$item]['Field'] . '" id="' . $cols[$item]['Field'] .'" rows="10" cols="50"></textarea>';
            break;

          case (preg_match("(int)", $cols[$item]['Type']) ? $cols[$item]['Type'] : !$cols[$item]['Type']):
            $form .= "<label for='" . $cols[$item]['Field'] . "'>$name</label><input type='text' name='" . $cols[$item]['Field'] . "' id='"
                  . $cols[$item]['Field'] . "' /><br />";
            break;
  ...
  protected function create_description( $text ) {
    return '<span class="desc">'.$text.'</span>';
  }
  ...
  public function clear() {
    unset($this->form_target, $this->form_method, $this->form_id, $this->
      form_fields, $this->form_result);
  }
