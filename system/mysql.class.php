...
{	private static $instance = false;
  ...
  function db_query($sql=NULL) { //start query function
		$this->source = $sql;
		$this->row_count = 0;
    if(!empty($sql)) {   // if $sql task is not empty, then proceed

      $resource = mysql_query($sql,$this->connection_id);

			if(!empty($resource)) { // if resource is filled then return it
    ...
    } else {   // else make an error and return false
      $this->error = "Task was empty";
      return false;
    }
  } // end function

  function select($cols,$table,$where=NULL,$order=NULL,$group=NULL,$start=0,$limit=0) { // create select query

    $cols = explode(",",$cols);
    $sql = "SELECT ";

    for($i=0;$i<count($cols);$i++) { // filing the query with col names
        $sql .= $cols[$i].", ";
    }
    ...
    $sql .= ' FROM '.$table.' ';
   	...
   	$sql .= (!empty($where)?'WHERE '.$where.' ' :null); // when a WHERE is, then set it, else make free space
  	$sql .= (!empty($group)?'GROUP BY '.$group.' ' :null);   // when a GROUP is set, then set it, else close the query
  	$sql .= (!empty($order)?'ORDER BY '.$order.' ' :null);   // when ORDER is set, then set it, else close the query
  	$sql .= (!empty($limit)?'LIMIT '.$start.','.$limit:';');   // when a LIMIT is set, then set it, else close the query
	...
	function select_row($cols, $table, $where=NULL, $order=NULL, $group=NULL) { // create select query
    ...
    $sql = substr($sql,0,-2);
		if( substr_count( $table, ">") > 0 ) {
			...
			$table = '`'.$this->table_prefix.str_replace('`',null,$table).'`';
		}
